...
export function MotorcycleCard({ motorcycle, isFavorite = false, onToggleFavorite, index = 0 }: MotorcycleCardProps) {
  return (
    <motion.div
    ...
    >
      <Link href={`/listing/motorcycle/${motorcycle.id}`} className="group block">
              ...
              >
                <Heart
                  className={`h-4 w-4 transition-colors duration-150 ${
                    isFavorite ? "text-red-500 fill-red-500" : "text-slate-500 dark:text-slate-400"
                  }`}
                />
              </button>
            )}
