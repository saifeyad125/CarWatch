...
def assign_price_segment(df):
    prices = np.expm1(df[TARGET])
    seg = pd.Series("budget", index=df.index)
    seg[prices > 200_000] = "mid"
    seg[prices > LUX_THRESHOLD] = "luxury"
    return seg


def split_train_calibration(train_df):
    segments = assign_price_segment(train_df)
    train_proper, cal_df = train_test_split(
        train_df, test_size=0.20, random_state=RANDOM_SEED, stratify=segments,
    )
    train_proper = train_proper.reset_index(drop=True)
    cal_df = cal_df.reset_index(drop=True)
    return train_proper, cal_df
...
def train_stage1(train_proper, test_df, cal_df):
    X_train = train_proper[ALL_FEATURES]
    y_train = train_proper[TARGET]
    X_test  = test_df[ALL_FEATURES]
    X_cal   = cal_df[ALL_FEATURES]
    ...
    # train final models on full train_proper using median best iteration from CV
    print(f"  Training final Stage 1 models on full train_proper...")
    final_models = {}
    for name, params in models_config.items():
    ...
    # CQR requires calibration and test scored by the exact models used at inference
    cal_preds = {}
    test_preds = {}
    for name in models_config:
        cal_preds[name] = final_models[name].predict(X_cal)
    ...
    return (final_models, oof_preds, test_preds, cal_preds,
            oof_sigma, test_sigma, cal_sigma, s1_metrics, s1_segments)


# stage 2: luxury residual correction

def prepare_stage2_data(train_proper, oof_mu, oof_sigma):
    df = train_proper.copy()
...
# CQR conformal calibration (Romano et al. 2019)
...
def calibrate_cqr(q_low_final, q_high_final, y_true_log, alpha=0.10):
    ...
    # order statistic index (Romano et al. 2019, Section 2.2)
    ...
    # sanity check: coverage on calibration set itself
    q_low_adj = q_low_final - Q
    q_high_adj = q_high_final + Q
    cal_coverage = float(np.mean((y_true_log >= q_low_adj) & (y_true_log <= q_high_adj)))

    print(f"  CQR calibration:")
    ...
    print(f"    Calibration set coverage: {cal_coverage*100:.2f}% (target: {(1-alpha)*100:.0f}%)")

    return Q, cal_coverage
...
def save_artifacts(final_models, stage2_model, stage2_features, tau, median_iters,
                   Q, cal_coverage, n_cal, s1_metrics, s1_segments, ts_metrics,
                   ts_segments, cqr_metrics, test_df, final_log, q05_cqr, q95_cqr,
    ...
    # save models
    final_models["point"].booster_.save_model(str(OUTPUT_DIR / "lightgbm_cqr_stage1.txt"))
    final_models["q05"].booster_.save_model(str(OUTPUT_DIR / "lightgbm_cqr_q05.txt"))
    final_models["q95"].booster_.save_model(str(OUTPUT_DIR / "lightgbm_cqr_q95.txt"))
    stage2_model.booster_.save_model(str(OUTPUT_DIR / "lightgbm_cqr_stage2.txt"))

    print(f"\n  Models saved to {OUTPUT_DIR}/")

    cqr_metrics["calibration_set_coverage"] = cal_coverage

    cal_info = {
        "calibration_method": "Conformalized Quantile Regression (Romano et al. 2019)",
        "conformal_Q": Q,
    ...
    cal_path = OUTPUT_DIR / "lightgbm_cqr_calibration.json"
    ...
    out_csv = OUTPUT_DIR / "test_predictions_lgbm_cqr.csv"
...
def main():
    print("=" * 80)
    print("LightGBM Two-Stage + CQR (Romano et al. 2019)")
    print("=" * 80)

    print("\n[1/6] Loading data...")
    train_df, test_df = load_data()
    train_proper, cal_df = split_train_calibration(train_df)
    print(f"  Train proper: {len(train_proper):,}  Calibration: {len(cal_df):,}  Test: {len(test_df):,}")

    seg_counts = assign_price_segment(cal_df).value_counts()
    print(f"  Calibration segments: {dict(seg_counts)}")

    print("\n[2/6] Stage 1: LightGBM point + quantile models...")
    (final_models, oof_preds, test_preds, cal_preds,
     oof_sigma, test_sigma, cal_sigma,
     s1_metrics, s1_segments) = train_stage1(train_proper, test_df, cal_df)

    print("\n[3/6] Stage 2: Luxury residual correction...")
    lux_df, stage2_features = prepare_stage2_data(
        train_proper, oof_preds["point"], oof_sigma,
    )
    stage2_model, best_tau, median_iters, stage2_features = train_stage2(
        lux_df, stage2_features,
    )

    print("\n[4/6] CQR conformal calibration...")
    cal_final_log, cal_q05_final, cal_q95_final, cal_w, _ = predict_two_stage(
        cal_preds["point"], cal_preds["q05"], cal_preds["q95"], cal_sigma,
        cal_df, stage2_model, stage2_features, best_tau,
    )

    Q, cal_coverage = calibrate_cqr(
        cal_q05_final, cal_q95_final, cal_df[TARGET].values, alpha=0.10,
    )
    ...
    save_artifacts(
        ...
        Q, cal_coverage, len(cal_df), s1_metrics, s1_segments, ts_metrics,
        ts_segments, cqr_metrics, test_df, final_log, q05_cqr, q95_cqr,
        gate_w, sigma_adj, price_low_cqr, price_high_cqr,
    )
