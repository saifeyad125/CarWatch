...
export default function DealerCarsBrowsePage() {
  ...
  const [allListings, setAllListings] = useState<DealerCarCardData[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchListings = async () => {
      try {
        ...
        const data = await apiRequest<{ listings: DealerCarCardData[]; total: number }>(
          `${API_ENDPOINTS.dealerCars.list}?${params}`,
        ...
        setError("Failed to load dealer listings. Please try again later.");
      } finally {
  ...
  useEffect(() => {
    if (selectedMake) {
      apiRequest<{ models: string[] }>(API_ENDPOINTS.dealerCars.models(selectedMake))
  ...
  useEffect(() => {
    if (selectedMake && selectedModel) {
      apiRequest<{ trims: string[] }>(API_ENDPOINTS.dealerCars.trims(selectedMake, selectedModel))
  ...
  useEffect(() => {
    setFavorites(getFavoritesByType("dealer"));
  }, []);

  const handleToggleFavorite = (carId: number) => {
    toggleFav("dealer", carId);
    setFavorites(getFavoritesByType("dealer"));
  };

  const [allBrands, setAllBrands] = useState<string[]>([]);
  useEffect(() => {
    apiRequest<{ brands: string[] }>(API_ENDPOINTS.dealerCars.brands)
  ...
  const activeFilters = [selectedMake, selectedModel, selectedFuelType, selectedYear, selectedTrim, priceMin, priceMax].filter(Boolean);
    ...
    <div className="flex flex-col h-full bg-background overflow-hidden">
      <header className="shrink-0 h-16 border-b border-border/40 bg-card/80 backdrop-blur-nav px-4 md:px-6 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-foreground tracking-tight">Dealer Certified Cars</h1>
          {!isLoading && (
            <Badge variant="secondary" className="text-xs font-medium">
              {totalListings} certified cars
            </Badge>
          )}
        </div>
        {user ? (
      ...
      {showFilters && (
        <div className="fixed inset-0 z-0 md:hidden" onClick={() => setShowFilters(false)} />
      )}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6 pb-safe">
          ...
          {!isLoading && !error && allListings.length === 0 && (
            <div className="text-center py-16">
              ...
              <h3 className="font-semibold text-foreground mb-1">No dealer cars found</h3>
          ...
          {!isLoading && !error && totalPages > 1 && (
            <div className="flex items-center justify-center gap-1.5 pt-8 pb-24 md:pb-8">
              <Button variant="outline" size="sm" disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="h-9 w-9 p-0">
              ...
              {getPageNumbers(currentPage, totalPages).map((page, i) =>
                page === "..." ? (
                  ...
                  <Button key={page} variant={page === currentPage ? "default" : "outline"} size="sm" onClick={() => goToPage(page as number)} className="h-9 w-9 p-0 text-sm">
                    {page}
                  </Button>
                )
              )}
              <Button variant="outline" size="sm" disabled={currentPage === totalPages} onClick={() => goToPage(currentPage + 1)} className="h-9 w-9 p-0">
